...
async function callGroqAPI(message) {
    try {
        ...
        try {
            ...
            console.error('[ERROR] API returned non-JSON response:', text);
            return `❌ Erro na API: resposta inválida do servidor (${response.status})`;
        }
        ...
        if (!response.ok) {
            ...
            return `❌ Erro da API: ${data.error || 'Erro desconhecido'}`;
        }
